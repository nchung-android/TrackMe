...
class SessionRepositoryImpl @Inject constructor(
    ...
    override suspend fun save(session: SessionModel) =
        sessionDAO.insert(sessionMapper.toDTO(session))

    override suspend fun delete(session: SessionModel) =
        sessionDAO.delete(sessionMapper.toDTO(session))

    override fun getLatestSession(): LiveData<SessionModel?> =
        Transformations.map(sessionDAO.getLatestSession()) {
    ...
    override fun getSession(id: Int): LiveData<SessionModel?> =
        Transformations.map(sessionDAO.getSession(id)) {
            sessionMapper.fromDTO(it ?: return@map null)
        }
